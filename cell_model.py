...
+ auxiliary class to enable resource-aware simulation of synthetic gene expression in the cell
'''
# By Kirill Sechkar
...
# scaling factor to omit modelling housekeeping genes
def Q_calc(par, e, R, h, prodeflux):
    return 1 / (1 - par['phi_q'] * (1 - prodeflux / (e * R * (par['K_D'] / (par['K_D'] + h)))))
...
class CellModelAuxiliary:
    ...
    def __init__(self):
        ...
        self.gene_colours = {'a': "#EDB120", 'r': "#7E2F8E", 'q': '#C0C0C0', # colours for metabolic, riboozyme, housekeeping genes
                        'het': "#0072BD", 'h': "#77AC30"}               # colours for heterologous genes and intracellular chloramphenicol level
    ...
    def default_params(self):
        ...
        params['K_D'] = 1300.0 # chloramphenicol-ribosome dissociation constant (nM) [5]
        params['K_C'] = (1/3) / 60 # dissociation constant for chloramphenicol removal by the cat (chloramphenicol resistance) protein, if present (nM*h) [5]
        ...
        # FITTED PARAMETERS
        params['a_a'] = 398196.0361 # metabolic gene transcription rate (/h)
        params['a_r'] = 271626.6387 # ribosomal gene transcription rate (/h)
        params['K_nu'] = 2095.55 # TRNA AMENDMENT # 1053.35 # 5992.78  # tRNA charging rate Michaelis-Menten constant (nM)
        params['K_e'] = 2095.55 # TRNA AMENDMENT # 1053.35 # 5992.78 # translation elongation rate Michaelis-Menten constant (nM)
        params['nu_max'] = 4405.67  # max tRNA amioacylation rate (/h)
        return params
    ...
    def plot_protein_masses(self,ts,xs,
        ...
        if(len(circuit_genes)!=0):
            ...
            top_line = bottom_line + np.sum(xs[:,8+len(circuit_genes):8+len(circuit_genes)*2]*np.array(self.synth_gene_params_for_jax(par,circuit_genes)[2],ndmin=2),axis=1)
    ...
    def plot_native_concentrations(self,ts,xs,
        ...
        h_figure.line(x='t',y='h', source=source, line_width=1.5, line_color=self.gene_colours['h'], legend_label='h') # plot intracellular chloramphenicol concentration
    ...
    def get_e_l_Fr_nu_psi_T_D_Dnohet(self, t, x,
        ...
        prodeflux = jnp.sum(jnp.atleast_2d(d_het)*jnp.ones((len(e),1)) *
                            jnp.atleast_2d(n_het)*jnp.ones((len(e),1)) *
                            x_het[:,len(circuit_genes):2*len(circuit_genes)],axis=1)  # heterologous protein degradation flux

        # resource competition denominator
        Q = Q_calc(par,e, R, h, prodeflux)  # scaling factor to omit modelling housekeeping genes
        D = ((par['K_D'] + h) / par['K_D']) * \
            (1 + Q * (m_a / k_a + m_r / k_r + (x_het[:,0:len(circuit_genes)] / k_het).sum(axis=1)))
...
def ode(t,x,circuit_ode,args):
    ...
    prodeflux = jnp.sum(d_het * n_het * x[8+num_circuit_genes:8+num_circuit_genes*2]) # heterologous protein degradation flux
    x_het_div_k_het = x[8:8+num_circuit_genes] / k_het  # heterologous protein synthesis flux

    # resource competition denominator
    Q = Q_calc(par, e, R, h, prodeflux)  # scaling factor to omit modelling housekeeping genes
    D = ((par['K_D'] + h) / par['K_D']) * \
        (1 + Q*(m_a / k_a + m_r / k_r + x_het_div_k_het.sum()))
    B = R * (par['K_D'] / (par['K_D'] + h) - 1 / D)  # actively translating ribosomes (inc. those translating housekeeping genes)
    ...
    # chloramphenicol degradation term - only present if the circuit contains a cat gene
    deg_h = jax.lax.select(par['cat_gene_present']==1, h*x[circuit_name2pos['p_cat']]/par['K_C'], 0.0)
    ...
    return jnp.array([
        ...
        par['diff_h'] * (par['h_ext'] - h) - deg_h - l*h
    ] +
...
def main():
    ...
    par['a_switch'] = 60.0  # promoter strength (unitless)
    ...
    par['K_switch'] = 400.0  # Half-saturation constant for the self-activating switch gene promoter (nM)
    par['eta_switch'] = 2

    # Hill coefficient for the self-activating switch gene promoter (unitless)
    par['baseline_switch'] = 0.05  # Baseline value of the switch gene's transcription activation function
    par['p_switch_ac_frac'] = 0.75  # active fraction of protein (i.e. share of molecules bound by the inducer)

    # culture medium
    par['s'] = 0.5
    par['h_ext'] = 10.5 * (10.0 ** 3)

    # for now, no integrase action
    #par['k_sxf'] = 0.0
    ...
    bkplot.output_file(filename="sundry plots/cellmodel_sim.html", title="Cell Model Simulation") # set up bokeh output file
    ...
    bkplot.output_file(filename="sundry plots/circuit_sim.html", title="Synthetic Gene Circuit Simulation") # set up bokeh output file
