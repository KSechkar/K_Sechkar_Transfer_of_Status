...
function params=cell_params(~)
    ...
    % chloramphenicol diffusion and action
    params('h_ext')=0; % external chloramphenicol concentration (nM)
    params('diff_h') = 90.0*60;  % chloramphenicol diffusion coefficient through the cell membrane (1/h)
    params('K_D') = 1300; % Cm-ribosome binding dissociation constant (nM)
