...
function [l, e, phi_r,T]=get_lephirT(sim,ss)
    ...
    k_a=sim.form.k(e,par('k+_a'),par('k-_a'),par('n_a')); % ribosome-mRNA dissociation constant (metab. genes)
    k_r=sim.form.k(e,par('k+_r'),par('k-_r'),par('n_r')); % ribosome-mRNA dissociation constant (rib. genes)
    D=(par('K_D')+h)/par('K_D').*...
        (1+(m_a./k_a+m_r./k_r)./...
